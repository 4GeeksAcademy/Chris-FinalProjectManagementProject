...
def create_client():
    ...
    client = Client(email=email, password=password, is_active=True, first_name=first_name,
                    last_name=last_name, phone_number=phone_number, about_me=about_me)
    db.session.add(client)
    db.session.commit()
    response_body = {"msg": "client successfully created",
...
def update_client(client_id):
    ...
    response_body = {"msg": "client successfully edited",
                     "client": client.serialize()}
    return jsonify(response_body), 201
...
def create_project_manager():
    ...
    if project_manager:
        return jsonify({"msg": "account associted with that email already exists"}), 409
    project_manager = ProjectManager(email=email, password=password, is_active=True, experience=experience,
                                     first_name=first_name, last_name=last_name, phone_number=phone_number, skills=skills)
    db.session.add(project_manager)
    db.session.commit()
    response_body = {"msg": "projectmanager successfully created",
                     "projectmanager": project_manager.serialize()}
    return jsonify(response_body), 201
...
def update_project_manager(project_manager_id):
    ...
    project_manager = ProjectManager.query.filter_by(
        id=project_manager_id).one_or_none()
    ...
    response_body = {"msg": "project_manager successfully edited",
                     "project_manager": project_manager.serialize()}
    return jsonify(response_body), 201
...
def create_invoice():
    date_issued = request.json.get("date_issued", None)
    date_due = request.json.get("due_date", None)
    client = request.json.get("client", None)
    project = request.json.get("project", None)
    amount = request.json.get("amount", None)
    tax_rate = request.json.get("tax_rate", None)
    status = request.json.get("status", None)
    notes = request.json.get("notes", None)
    if date_issued is None or date_due is None or client is None or project is None or amount is None or tax_rate is None or status is None or status is None or notes is None:
        return jsonify({"msg": "some fields are missing in your request"}), 400

    invoice = Invoice(date_issued=date_issued, date_due=date_due, client=client,
                      project=project, amount=amount, tax_rate=tax_rate, status=status, notes=notes)
    db.session.add(invoice)
    db.session.commit()
    response_body = {"msg": "invoice successfully created",
